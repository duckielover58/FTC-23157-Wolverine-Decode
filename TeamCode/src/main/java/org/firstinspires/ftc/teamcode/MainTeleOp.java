...
//        PinpointDrive drive = new PinpointDrive(hardwareMap, new Pose2d(GlobalVariable.nearBlue.IntakeEnd3X, GlobalVariable.nearBlue.IntakeEnd3Y, Math.toRadians(0)));
        ...
        while (opModeIsActive()) {
            ...
            if (cG2.right_trigger >= 0.1) {
                ...
                lodk(mainTag);
                flywheelPID(close);
                velHoodPos = (flywheel2.getVelocity() * (targetHoodClose/close)); // -0.1 * result.getDistance();
                telemetry.addData("Vel Hood Pos: ", velHoodPos);
                telemetry.addData("Velocity: ", flywheel2.getVelocity());
                runningActions.add(new SequentialAction(hood.setHoodPosShoot()));
            } else if (cG2.left_trigger >= 0.1) {
                onShootIndex = true;
                lodk(mainTag);
                flywheelPID(GlobalVariable.far);
                velHoodPos = flywheel2.getVelocity() * (targetHoodFar/close);
                telemetry.addData("Vel Hood Pos: ", velHoodPos);
                runningActions.add(new SequentialAction(hood.setHoodPosShoot()));
            } else {
                onShootIndex = false;
